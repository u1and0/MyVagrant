タイトル「Archlinux日本語GUI環境セットアップスクリプト」
...
また、shellscriptを追っていけば単純に「Archlinuxことはじめ」としても使えると思います。(ネットワーク設定など、gitいれるとかまではterrywangさんが既にカスタマイズしてくれている模様)
...
* pacmanの強化
    * powerpillの導入 
    * reflectorを使って日本圏のサーバーをpacman.d/mirrorlistに書き込み
        * pacmanでインストールするとき、ダウンロードが速なります。
...
`bootstrap.sh`と`Vagrantfileが同じディレクトリにある必要があります。`
...
日本の10個のミラーをダウンロード速度順にソートし、/etc/pacman.d/mirrorlist ファイルを上書きします。
> ミラーそもそも10個もない…アメリカは少なくとも200もあるのに
